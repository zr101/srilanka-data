...
Each dataset lives on its own `data_<dataset>` branch: per-doc dirs (`original.pdf` + `doc.json` + `clean.json`), regenerated indexes (`docs_all.tsv`, `docs_last100.tsv`, `summary.json`), and a `latest.json` pointer. Parsers are fixture-tested; raw originals are kept so clean data can be rebuilt retroactively.
